...
namespace FeedApp.Application.Services
{
        ...
        ILogger<FeedService> logger) : IFeedService
    {
        public async Task<PagedResponse<FeedResponseDto>> GetFeedsAsync(FeedListRequest request, CancellationToken ct = default)
        {
            ...
            var (items, totalCount) = await feedRepository.GetPagedAsync(
                request.Page, request.PageSize, request.UserId, request.FeedType, request.SearchTerm, ct);
        ...
        public async Task<FeedResponseDto> GetFeedByIdAsync(Guid id, CancellationToken ct = default)
        {
            ...
            var details = await feedRepository.GetDetailsByIdAsync(id, ct)
        ...
        public async Task<FeedResponseDto> CreateFeedAsync(CreateFeedRequest request, Guid userId, CancellationToken ct = default)
        {
            ...
            // Verify user exists
            if (!await userRepository.ExistsByIdAsync(userId, ct))
        ...
        public async Task<(byte[] data, string contentType)> GetFeedImageAsync(Guid id, CancellationToken ct = default)
        {
            ...
            var (imageData, mimeType) = feed switch
            {
                ImageFeed img => (img.ImageData, img.ImageMimeType),
                VideoFeed vid => (vid.ImageData, vid.ImageMimeType),
                _ => throw new ValidationException("VALIDATION_ERROR",
